...
int *e, *le,  // current position in emitted code e:目前機器碼指標 le:上一行機器碼指標
    *id,      // currently parsed identifier 目前ID
    *sym,     // symbol table (simple list of identifiers) 符號表
    tk,       // current token 目前的token
    ival,     // current token value 目前token的值
    ty,       // current expression type 目前運算式的型態
    loc,      // local variable offset 區域變數位移
    line,     // current line number 目前行數
    src,      // print source and assembly flag 印出原始碼
    debug;    // print executed instructions 印出執行指令

// tokens and classes (operators last and in precedence order) 將tokens和classes以優先順序排列
enum { //0~127以該字母表示，128開始用代號表示
...
// identifier offsets (since we can't create an ident struct) 因為沒有寫出id的結構體，所以用offset代替
...
void next() // 詞彙解析
{
  ...
  while (tk = *p) {
    ...
    else if (tk == '#') { // 取得函式庫那一類的程式
      while (*p != 0 && *p != '\n') ++p;
    }
    else if ((tk >= 'a' && tk <= 'z') || (tk >= 'A' && tk <= 'Z') || tk == '_') { // 取得變數
      pp = p - 1;
    ...
    else if (tk >= '0' && tk <= '9') { // 取得數字
      if (ival = tk - '0') { while (*p >= '0' && *p <= '9') ival = ival * 10 + *p++ - '0'; } // 十進位
      else if (*p == 'x' || *p == 'X') {
        while ((tk = *++p) && ((tk >= '0' && tk <= '9') || (tk >= 'a' && tk <= 'f') || (tk >= 'A' && tk <= 'F'))) // 十六進位
    ...
    else if (tk == '/') { // 取得註解(這裡只能讀取以"//"寫的註解，"/*  */"的無法讀取)
      if (*p == '/') {
        ++p;
        while (*p != 0 && *p != '\n') ++p; // 略過註解
      }
    ...
    else if (tk == '\'' || tk == '"') { // 取得字元或字串
      pp = data;
      while (*p != 0 && *p != tk) {
        if ((ival = *p++) == '\\') {
          if ((ival = *p++) == 'n') ival = '\n'; //因為"\n"不是字元或字串，所以這裡要把他去掉
        }
        if (tk == '"') *data++ = ival; //將字串放入資料段
      }
      ++p;
...
void expr(int lev) // 運算式
{
  ...
  else if (tk == Sizeof) { // 處理sizeof()，()中的變數形式可能為int,char或ptr
  ...
  while (tk >= lev) { // "precedence climbing" or "Top Down Operator Precedence" method 處理運算符解析順序
    t = ty;
    if (tk == Assign) {
      next();
...
int main(int argc, char **argv) // 主程式
{
  ...
  *--sp = (int)t;
  ...
  while (1) {
    ...
    else if (i == JMP) pc = (int *)*pc;                                   // jump 跳躍指令
    else if (i == JSR) { *--sp = (int)(pc + 1); pc = (int *)*pc; }        // jump to subroutine 跳至副程式
    ...
    else if (i == OPEN) a = open((char *)sp[1], *sp); // 開啟檔案
    else if (i == READ) a = read(sp[2], (char *)sp[1], *sp); // 讀取檔案
    else if (i == CLOS) a = close(*sp); // 關閉檔案
